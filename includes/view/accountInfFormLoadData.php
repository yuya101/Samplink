...
$sql = "select member_login from member_login where memberID=".$_SESSION['mLoginID'];
$result = $mQuery->getResultAll($sql);

foreach($result as $r)
{
	$memberEmail = $r['member_login'];
}
...
if($num > 0)
{
	...
	$sql = "select pictureName1 from member_picture where memberID=".$_SESSION['mLoginID']." and pictureName1!='-'";
	$num = $mQuery->checkNumRows($sql);
	...
	if($num > 0)
	{
		...
		foreach($result as $r)
		{
			$memberPicture = '<img src="upload/'.$r['pictureName1'].'" width="107" height="107" class="presentImg">';
		}
	}
	else
	{
		$memberPicture = '<img src="upload/default.jpg" width="107" height="107" class="presentImg">';
	}
}
else
{
